    ...
    <Card className="max-w-4xl mx-auto p-6 md:p-8">
      ...
      <div className="grid lg:grid-cols-2 gap-8 items-start">
        ...
        <div className="bg-brand-bg/50 p-4 rounded-lg">
          ...
          <div className="mb-6">
            <label className="block font-semibold mb-2">Reserve Ratio: <span className="text-brand-primary font-mono">{reserveRatio}%</span></label>
            <p className="text-xs text-brand-text-secondary mb-2">This is the percentage of deposits the bank must keep and cannot lend out.</p>
            <input
              type="range"
              min="5"
              max="50"
          ...
          <div className="text-center space-y-2">
            <p className="text-brand-text-secondary">Money Multiplier:</p>
            <p className="text-2xl font-bold text-brand-primary">{moneyMultiplier.toFixed(1)}x</p>
            <p className="text-brand-text-secondary mt-2">Max Money Supply from ${initialDeposit}:</p>
        ...
        {/* Visualization */}
        <div>
          <h3 className="text-xl font-semibold text-white mb-4 text-center">Money Creation Cycle</h3>
          <div className="space-y-2">
            <div className="p-3 bg-brand-surface rounded-lg text-center">
              Initial Deposit: <span className="font-bold text-white">${initialDeposit.toLocaleString()}</span>
            </div>
            {steps.slice(0, step).map((s, index) => (
              <Card key={index} className="p-3 bg-brand-bg/50 animate-fade-in">
                <p className="font-bold text-brand-primary">Round {s.round}</p>
                <div className="text-sm space-y-1 mt-1">
                  <p>Bank receives <span className="font-mono text-white">${s.deposit.toFixed(2)}</span></p>
                  <p>Holds <span className="font-mono text-yellow-400">{reserveRatio}%</span> in reserve: <span className="font-mono text-yellow-400">${s.reserve.toFixed(2)}</span></p>
                  <p>Lends out <span className="font-mono text-green-400">${s.loan.toFixed(2)}</span></p>
                </div>
              </Card>
            ))}
          </div>
          {step < steps.length && (
            <div className="text-center mt-4">
              <Button onClick={() => setStep(s => s + 1)}>Show Next Round</Button>
            </div>
          )}
        </div>
      </div>
      ...
      <Card className="mt-8 p-4 bg-brand-surface text-center">
        <h3 className="text-lg font-bold text-white">Summary of Current Cycle</h3>
        <div className="grid grid-cols-3 gap-4 mt-2">
            <div>
                <p className="text-sm text-brand-text-secondary">Initial Deposit</p>
                <p className="text-xl font-bold text-white">${initialDeposit.toLocaleString()}</p>
            </div>
            <div>
                <p className="text-sm text-brand-text-secondary">New Money Created</p>
                <p className="text-xl font-bold text-brand-secondary">${(step > 0 ? steps[step - 1].newMoney : 0).toFixed(2)}</p>
            </div>
            <div>
                <p className="text-sm text-brand-text-secondary">Total Money Supply</p>
      ...
      {!showQuiz && step > 0 && (
          <div className="text-center mt-6">
              <Button onClick={() => setShowQuiz(true)} variant="secondary">I understand, take the quiz</Button>
          </div>
      )}
