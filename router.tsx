...
function SimulationPage() {
    ...
    // Helper to convert 'seed-phrase' to 'SeedPhraseSim'
    const simIdToComponent = (id) => {
      return id.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join('') + 'Sim';
    }

    // Dynamically import the simulation component
    const SimComponent = lazy(() => import(`./components/simulations/${simIdToComponent(currentSim.id)}.tsx`));
